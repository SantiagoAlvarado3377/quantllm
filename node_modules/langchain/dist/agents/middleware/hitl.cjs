...
const InterruptOnConfigSchema = zod_v3.z.object({
	...
	argumentsSchema: zod_v3.z.record(zod_v3.z.any()).optional()
});
...
function humanInTheLoopMiddleware(options) {
	const createActionAndConfig = async (toolCall, config, state, runtime) => {
		...
		*/
		...
		if (config.argumentsSchema) reviewConfig.argumentsSchema = config.argumentsSchema;
		return {
			actionRequest,
			reviewConfig
		};
	...
	const processDecision = (decision, toolCall, config) => {
		...
		if (decision.type === "edit" && allowedDecisions.includes("edit")) {
			...
			*/
			...
			if (!editedAction.arguments || typeof editedAction.arguments !== "object") throw new Error(`Invalid edited action for tool "${toolCall.name}": arguments must be an object`);
			return {
				revisedToolCall: {
					type: "tool_call",
